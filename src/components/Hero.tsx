      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6 md:gap-12">
        ...
        <div className="flex justify-center md:justify-end w-full md:w-1/2 order-1 md:order-2">
          <div className="relative">
            ...
            <div className="relative w-[clamp(200px,75vw,320px)] h-[clamp(200px,75vw,320px)] sm:w-72 sm:h-72 md:w-80 md:h-80 lg:w-96 lg:h-96">
              <div className="hero-image-container relative overflow-hidden rounded-2xl border-2 sm:border-4 border-[hsl(var(--primary))]/30 shadow-2xl bg-[hsl(var(--card))] w-full h-full">
                {!imgError ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src="/assets/me-2-enhance8x.webp"
                    alt="Daniel Poh Ting Fong"
                    className="object-cover object-top w-full h-full transition-transform duration-700 hover:scale-110"
                    loading="lazy"
                    onError={() => setImgError(true)}
                  />
                ) : (
              ...
              {/* Floating decorative dots — pushed inward on mobile to avoid clipping */}
              <div className="absolute -top-2 -right-2 sm:-top-3 sm:-right-3 md:-top-4 md:-right-4 w-3 h-3 sm:w-5 sm:h-5 md:w-8 md:h-8 bg-[hsl(var(--primary))] rounded-full animate-bounce-slow opacity-80"></div>
              <div className="absolute -bottom-2 -left-2 sm:-bottom-3 sm:-left-3 md:-bottom-4 md:-left-4 w-2 h-2 sm:w-3 sm:h-3 md:w-6 md:h-6 bg-[hsl(var(--primary))]/60 rounded-full animate-bounce-slow-delayed opacity-60"></div>
              <div className="absolute top-1/2 -left-1 sm:-left-3 md:-left-6 w-1.5 h-1.5 sm:w-2.5 sm:h-2.5 md:w-4 md:h-4 bg-[hsl(var(--primary))]/40 rounded-full animate-float opacity-40"></div>
            </div>
          </div>
        </div>
      </div>
